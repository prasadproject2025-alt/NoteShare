    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <form id="share-form" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="share_image" class="form-label">Upload Note Image</label>
                    <input type="file" class="form-control" id="share_image" accept="image/*" required>
                </div>
...
function loadSlotNotes() {
    ...
    firebase.database().ref('shared_notes').orderByChild('batch').equalTo(selectedSlot)
        .once('value').then(snapshot => {
            ...
            if (notes.length === 0) {
                notesDiv.innerHTML = '<p class="text-muted">No shared notes for slot ' + selectedSlot + ' yet.</p>';
                return;
            }
...
function shareNotes() {
    const file = document.getElementById('share_image').files[0];
    if (!file) {
