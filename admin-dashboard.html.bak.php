...
function resetUserCoins(userId) {
    if (confirm('Are you sure you want to reset this user\'s coins to 0?')) {
        firebase.database().ref('users/' + userId + '/coins').set(0).then(() => {
...
function toggleBlockUser(userId, block) {
    ...
    firebase.database().ref('users/' + userId + '/blocked').set(block).then(() => {
...
function promptAddCoins(userId) {
    ...
    coinsRef.transaction(current => {
        ...
        if (error) {
            ...
            alert('Added ' + amount + ' coins to the user. New total: ' + (snapshot.val() || 0));
            loadUsers();
            loadStatistics();
        }
    });
